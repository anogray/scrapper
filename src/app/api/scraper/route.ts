...
const chromiumBlob =
  "https://scrapper-bay.vercel.app/chromium-v127.0.0-pack.tar";
  ...
  try {
    ...
    browser = await puppeteer.launch({
      args: isLocalChrome
        ...
        : [...chromium.args, "--hide-scrollbars", "--incognito", "--nosandbox"],
      // args: [
      //     ...chromium.args,
    ...
    let hrefs = await page.$$eval("a", (alinks) =>
      alinks.map((ele) => ele.href)
    );
    ...
    const extractedEncodedURL = hrefs[8]?.split("?url=")[1];
    if (extractedEncodedURL) {
      // Decode the extracted URL
      const decodedURL = decodeURIComponent(extractedEncodedURL);
      console.log({ extractedEncodedURL, decodedURL });
      ...
      return Response.json({ success: false, error: "No valid URL found" });
    }
  } catch (error: any) {
